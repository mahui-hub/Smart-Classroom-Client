    ...
    <el-row :gutter="40">
      <el-col :span="4">
        <div class="left-box">
          ...
          <div v-for="m in tiezifenleiList" :key="m.id" :label="m.fenleimingcheng" class="box" @click="view(m.id)">
            {{ m.fenleimingcheng }}
          </div>
        </div>
      </el-col>
        ...
        <div class="right-box">
          <div class="search_box">
            <el-form inline :model="search" size="mini">
              ...
              <el-form-item>
                <el-button type="primary" @click="addSubmit" icon="el-icon-plus" v-if="role != '管理员'">发帖子</el-button>
              </el-form-item>
            </el-form>
          </div>
          <el-row class="box-content" v-for="r in list" :key="r.id" @click.native="detail(r.id)" :gutter="20">
            <el-col :span="4">
    ...
    <el-dialog :title="headerTitle" :visible.sync="dialogVisible" size="mini">
      <div class="form-database-form">
        <el-form :model="form" ref="formModel" label-width="100px" status-icon validate-on-rule-change>
          ...
          <el-form-item label="帖子标题" prop="biaoti" required :rules="[{ required: true, message: '请填写标题' }]">
