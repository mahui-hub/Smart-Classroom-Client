<template>
  <div class="app">
    ...
    <div style="margin: 10px 0;text-align:right ">
      <el-button type="primary" @click="dianzan(map.id)" round>点赞 </el-button>
      <el-button type="success" @click="pinglun(map.id)" round>评论 </el-button>
      <el-button v-if="map.faburen == userName" type="warning" @click="edit(map.id)" round>编辑</el-button>
      <el-button v-if="map.faburen == userName" type="danger" @click="deleteItem(map)" round> 删除</el-button>
    </div>

    <e-forum>
      <e-forum-page :lists="replyList">
        <template v-slot:default="{ row, floor }">
          <e-forum-item :floor="floor" :id="row.id" :row="row" :addtime="row.addtime" :nickname="row.user.xuehao"
            :content="row.huifuneirong">
            <e-user slot="user" :nickname="row.user.xuehao" :headimg="row.user.touxiang">
              <dl class="cc">
    ...
    <el-dialog title="编辑帖子" :visible.sync="dialogVisible" size="mini">
      <div class="form-database-form">
        <el-form :model="form" ref="formModel" label-width="100px" status-icon validate-on-rule-change>
          ...
          <el-form-item label="帖子标题" prop="biaoti" required :rules="[{ required: true, message: '请填写帖子标题' }]">
            <el-input placeholder="请输入帖子标题" v-model="form.biaoti" />
          </el-form-item>
          <el-form-item label="帖子类型" prop="fenlei" required :rules="[{ required: true, message: '请选择帖子类型' }]">
    ...
    <el-dialog title="评论帖子" :visible.sync="dialogVisible2" size="mini">
      <div class="form-database-form">
        <el-form :model="form2" ref="formModel2" label-width="100px" status-icon validate-on-rule-change>
          <el-form-item label="帖子编号" prop="tiezibianhao">
