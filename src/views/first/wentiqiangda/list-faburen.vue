<template>
        ...
        <el-table border :data="list" style="width: 100%" highlight-current-row>
            <el-table-column type="index" label="#"></el-table-column> <!-- 序号 -->
                ...
                <template slot-scope="{row}">
                    <e-img :src="row.tupian" style="max-width:120px" />                </template>
                        ...
                        <el-table-column label="抢答人数"width="80">
                <template slot-scope="{row}">
                    {{  row.qiangdarenshu  }}                </template>
            </el-table-column>
                        <el-table-column label="已抢人数"width="80">
                <template slot-scope="{row}">
                    {{  row.yiqiangrenshu  }}                </template>
            </el-table-column>
                        <el-table-column label="附件">
                <template slot-scope="{row}">
                    <e-file-list v-model="row.fujian"></e-file-list>                </template>
            </el-table-column>
            ...
            <el-table-column label="操作">
                <template slot-scope="{row}">
                    <el-button-group>
                    <el-button @click="$goto('/admin/qiangdawentiadd?id='+row.id)" type="success" icon="el-icon-plus" size="mini">
                                抢答问题添加
                            </el-button>
                            <el-button type="info" @click="'/admin/qiangdawenti?wentiqiangdaid='+row.id" icon="el-icon-search" size="mini">
                                抢答问题查询
                            </el-button>
                                            <el-tooltip content="详情" placement="top">
                        <el-button @click="$goto({path:'/admin/wentiqiangdadetail',query:{id:row.id } })" icon="el-icon-info" type="info" size="mini"></el-button>
                        </el-tooltip>
                                            <el-tooltip content="编辑" placement="top">
                    <el-button icon="el-icon-edit" @click="$goto({path:'/admin/wentiqiangdaupdt',query:{id:row.id } })"
    ...
    export default {
        ...
        methods: {
            ...
            loadList( page ){
                ...
                this.$post(api.wentiqiangda.listfaburen , filter).then(res=>{
                    this.loading = false;
            ...
            deleteItem( row ){
                this.$confirm('确定删除数据？' , '提示',{ // 弹出 确认框提示是否要删除
                    ...
                    this.$post(api.wentiqiangda.delete , {// 提交后台
                        id:row.id
                    }).then(res=>{
                        this.loading = false;
                        if(res.code != api.code.OK){
