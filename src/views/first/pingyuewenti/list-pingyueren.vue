<template>
        ...
        <div class="form-search">
            <el-form @submit.prevent.stop :inline="true" size="mini">
                ...
                </el-form-item>                <el-form-item label="抢答人">
                
                    <el-input v-model="search.qiangdaren"></el-input>
                    
                </el-form-item>                <el-form-item label="评分">
...
-<el-input type="text" style="width:80px;" v-model="search.pingfen_end"></el-input>
                ...
                </el-form-item>                <el-form-item label="评阅简介">
            ...
            <el-table-column type="index" label="#"></el-table-column> <!-- 序号 -->
                ...
                <template slot-scope="{row}">
                    {{  row.faburen  }}                </template>
                        ...
                        <el-table-column label="抢答人"width="80">
                <template slot-scope="{row}">
                    {{  row.qiangdaren  }}                </template>
            </el-table-column>
                        <el-table-column label="评分"width="80">
                <template slot-scope="{row}">
                    {{  row.pingfen  }}                </template>
            </el-table-column>
                        <el-table-column label="评阅简介"width="130">
                <template slot-scope="{row}">
                    {{  row.pingyuejianjie  }}                </template>
            </el-table-column>
                        <el-table-column label="评阅人"width="80">
                <template slot-scope="{row}">
                    {{  row.pingyueren  }}                </template>
            </el-table-column>
            ...
            <el-table-column label="操作">
                <template slot-scope="{row}">
                    <el-button-group>
                        ...
                        <el-button @click="$goto({path:'/admin/pingyuewentidetail',query:{id:row.id } })" icon="el-icon-info" type="info" size="mini"></el-button>
                        </el-tooltip>
                                            <el-tooltip content="编辑" placement="top">
                    <el-button icon="el-icon-edit" @click="$goto({path:'/admin/pingyuewentiupdt',query:{id:row.id } })"
    ...
    export default {
        ...
        methods: {
            ...
            loadList( page ){
                ...
                this.$post(api.pingyuewenti.listpingyueren , filter).then(res=>{
                    this.loading = false;
            ...
            deleteItem( row ){
                this.$confirm('确定删除数据？' , '提示',{ // 弹出 确认框提示是否要删除
                    ...
                    this.$post(api.pingyuewenti.delete , {// 提交后台
                        id:row.id
                    }).then(res=>{
                        this.loading = false;
                        if(res.code != api.code.OK){
