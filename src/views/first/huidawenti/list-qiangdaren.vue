    ...
    <el-card class="box-card">
      ...
      <el-table border :data="list" style="width: 100%" highlight-current-row>
        ...
        <el-table-column label="回答附件">
          <template slot-scope="{ row }">
            <e-file-list v-model="row.huidafujian"></e-file-list>
          </template>
        </el-table-column>
        <el-table-column label="分数" width="80">
          <template slot-scope="{ row }"> {{ row.fenshu }} </template>
        </el-table-column>
          ...
          <template slot-scope="{ row }">
            <el-button-group>
              <!--     <el-button @click="$goto('/admin/pingyuewentiadd?id='+row.id)" type="success" icon="el-icon-plus" size="mini">
                            ...
                            <el-button type="info" @click="'/admin/pingyuewenti?huidawentiid='+row.id" icon="el-icon-search" size="mini">
                                评阅问题查询
                            </el-button>-->
...
export default {
  ...
  methods: {
    ...
    loadList(page) {
      ...
      this.$post(api.huidawenti.listqiangdaren, filter)
        .then((res) => {
          this.loading = false;
    ...
    deleteItem(row) {
      this.$confirm("确定删除数据？", "提示", {
        ...
        .then(() => {
          ...
          this.$post(api.huidawenti.delete, {
            // 提交后台
            id: row.id,
          })
            .then((res) => {
