<template>
        ...
        <div class="form-search">
            <el-form @submit.prevent.stop :inline="true" size="mini">
                ...
                </el-form-item>                <el-form-item label="题目">
                
                    <el-input v-model="search.timu"></el-input>
                    
                </el-form-item>                <el-form-item label="回答人">
                
                    <el-input v-model="search.huidaren"></el-input>
                    
                </el-form-item>                <el-form-item label="打分">
                
                    <el-input type="text" style="width:80px;" v-model="search.dafen_start"></el-input>
-<el-input type="text" style="width:80px;" v-model="search.dafen_end"></el-input>
            ...
            <el-table-column type="index" label="#"></el-table-column> <!-- 序号 -->
                ...
                <template slot-scope="{row}">
                    {{  row.faburen  }}                </template>
                        ...
                        <el-table-column label="回答人"width="80">
                <template slot-scope="{row}">
                    {{  row.huidaren  }}                </template>
            </el-table-column>
            ...
            <el-table-column label="操作">
                <template slot-scope="{row}">
                    <el-button-group>
                        ...
                        <el-button @click="$goto({path:'/admin/datipingyuedetail',query:{id:row.id } })" icon="el-icon-info" type="info" size="mini"></el-button>
                        </el-tooltip>
                                            <el-tooltip content="编辑" placement="top">
                    <el-button icon="el-icon-edit" @click="$goto({path:'/admin/datipingyueupdt',query:{id:row.id } })"
    ...
    export default {
        ...
        methods: {
            ...
            loadList( page ){
                ...
                this.$post(api.datipingyue.listhuidaren , filter).then(res=>{
                    this.loading = false;
            ...
            deleteItem( row ){
                this.$confirm('确定删除数据？' , '提示',{ // 弹出 确认框提示是否要删除
                    ...
                    this.$post(api.datipingyue.delete , {// 提交后台
                        id:row.id
                    }).then(res=>{
                        this.loading = false;
                        if(res.code != api.code.OK){
