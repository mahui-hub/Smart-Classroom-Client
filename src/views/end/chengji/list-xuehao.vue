<template>
  <div class="v-list" v-loading="loading" element-loading-text="加载中">
    ...
    <el-table border :data="list" style="width: 100%" highlight-current-row>
      ...
      <el-table-column label="教师评价成绩" align="center">
        <template slot-scope="{ row }">
          {{ row.jiaoshipingjiachengji }}
        </template>
      </el-table-column>
      <el-table-column label="随堂测试成绩" align="center">
        <template slot-scope="{ row }">
          {{ row.suitangceshichengji }}
        </template>
      </el-table-column>
      <el-table-column label="抢答问题成绩" align="center">
        <template slot-scope="{ row }">
...
export default {
  ...
  methods: {
    ...
    loadList(page) {
      ...
      this.page = page;
      // 筛选条件
      this.search.xuehao=localStorage.getItem('username')
      ...
      this.$post(api.chengji.listxuehao, filter)
        .then((res) => {
          this.loading = false;
