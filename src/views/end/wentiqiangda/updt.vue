<template>
    <div class="wentiqiangda-add" v-loading="loading">
        <el-card class="box-card">
            <div slot="header" class="clearfix updt">
                <el-page-header @back="$router.go(-1)" content="编辑问题抢答">
                </el-page-header>
            </div>
            <div class="form-database-form">
                ...
                <el-form :model="form" ref="formModel" label-width="130px" status-icon validate-on-rule-change>
                    <el-form-item label="编号" prop="bianhao" :rules="[{required:true, message:'请填写编号'}]">
                        <el-input placeholder="输入编号" style="width:250px;" v-model="form.bianhao" />
                    </el-form-item>

                    <el-form-item label="标题" prop="biaoti">
                        <el-input placeholder="输入标题" style="width:250px;" v-model="form.biaoti" />
                    </el-form-item>
                    ...
                    <el-form-item label="抢答人数" prop="qiangdarenshu"
                        :rules="[{validator:rule.checkNumber, message:'输入一个有效数字'}]">
                        <el-input placeholder="输入抢答人数" style="width:250px;" v-model="form.qiangdarenshu" />
                    </el-form-item>
        ...
        methods: {
            submit() {
                this.$refs.formModel.validate().then(res => {
                    ...
                    this.$post(api.wentiqiangda.update, form).then(res => {
                        this.loading = false;
            ...
            loadInfo() {
                ...
                this.loading = true;
                var form = this.form;
                this.$post(api.wentiqiangda.edit, {
                    id: this.id
                }).then(res => {
                    this.loading = false;
                    if (res.code == api.code.OK) {
