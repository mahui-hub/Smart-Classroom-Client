<template>
        ...
        <el-table border :data="list" style="width: 100%" highlight-current-row>
            ...
            <el-table-column label="操作">
                <template slot-scope="{row}">
                    <el-button-group>
                  <!--  <el-button @click="$goto('/admin/datiadd?id='+row.id)" type="success" icon="el-icon-plus" size="mini">
                                答题添加
                            </el-button>
                            <el-button type="info" @click="'/admin/dati?suitangceshiid='+row.id" icon="el-icon-search" size="mini">
                                答题查询
                            </el-button>-->
                                            <el-tooltip content="详情" placement="top">
                        <el-button @click="$goto({path:'/admin/suitangceshidetail',query:{id:row.id } })" icon="el-icon-info" type="info" size="mini"></el-button>
                        </el-tooltip>
                                            <el-tooltip content="编辑" placement="top">
                    <el-button icon="el-icon-edit" @click="$goto({path:'/admin/suitangceshiupdt',query:{id:row.id } })"
    ...
    export default {
        ...
        methods: {
            ...
            loadList( page ){
                ...
                this.$post(api.suitangceshi.listfaburen , filter).then(res=>{
                    this.loading = false;
            ...
            deleteItem( row ){
                this.$confirm('确定删除数据？' , '提示',{ // 弹出 确认框提示是否要删除
                    ...
                    this.$post(api.suitangceshi.delete , {// 提交后台
                        id:row.id
                    }).then(res=>{
                        this.loading = false;
                        if(res.code != api.code.OK){
