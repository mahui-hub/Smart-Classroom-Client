    ...
    <el-card class="box-card">
      ...
      <el-table border :data="list" style="width: 100%" highlight-current-row>
        ...
        <el-table-column label="教师" width="80">
          <template slot-scope="{ row }"> {{ row.jiaoshi }} </template>
        </el-table-column>
        ...
        <el-table-column label="操作">
          <template slot-scope="{ row }">
            <el-button-group>
              <el-button
                @click="$goto('/admin/shitiadd?id=' + row.id)"
                type="success"
                icon="el-icon-plus"
                size="mini"
              >
              ...
              <el-button
                type="info"
                @click="'/admin/shiti?tikuid=' + row.id"
                icon="el-icon-search"
                size="mini"
              >
                试题查询 </el-button
              ><el-button
                @click="$goto('/admin/jieguoadd?id=' + row.id)"
                type="success"
                icon="el-icon-plus"
                size="mini"
              >
              ...
              <el-button
                type="info"
                @click="'/admin/jieguo?tikuid=' + row.id"
                icon="el-icon-search"
                size="mini"
              >
                结果查询 </el-button
              ><el-button
                @click="$goto('/admin/kaoshijieguoadd?id=' + row.id)"
                type="success"
                icon="el-icon-plus"
                size="mini"
              >
              ...
              <el-button
                type="info"
                @click="'/admin/kaoshijieguo?tikuid=' + row.id"
                icon="el-icon-search"
                size="mini"
              >
                评价结果查询
              ...
              <el-tooltip content="详情" placement="top">
                <el-button
                  @click="
                    $goto({ path: '/admin/tikudetail', query: { id: row.id } })
                  "
                  icon="el-icon-info"
                  type="info"
                  size="mini"
              ...
              <el-tooltip content="编辑" placement="top">
                <el-button
                  icon="el-icon-edit"
                  @click="
                    $goto({ path: '/admin/tikuupdt', query: { id: row.id } })
                  "
                  type="warning"
                  size="mini"
                ></el-button>
