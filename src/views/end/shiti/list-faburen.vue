<template>
        ...
        <div class="form-search">
            <el-form @submit.prevent.stop :inline="true" size="mini">
                ...
                </el-form-item>                <el-form-item label="试题题目">
                
                    <el-input v-model="search.shititimu"></el-input>
                    
                </el-form-item>                <el-form-item label="类型">
                
                    <el-select v-model="search.leixing" ><el-option label="请选择" value=""></el-option><el-option label="单选题" value="单选题"></el-option>
<el-option label="多选题" value="多选题"></el-option>
        ...
        <el-table border :data="list" style="width: 100%" highlight-current-row>
            <el-table-column type="index" label="#"></el-table-column> <!-- 序号 -->
                ...
                <template slot-scope="{row}">
                    {{  row.shititimu  }}                </template>
            </el-table-column>
                        <el-table-column label="类型"width="80">
                <template slot-scope="{row}">
                    {{  row.leixing  }}                </template>
            </el-table-column>
                        <el-table-column label="答案">
                <template slot-scope="{row}">
                    <e-paper-view :vo="row" :map="row.daan"></e-paper-view>                </template>
            </el-table-column>
            ...
            <el-table-column label="操作">
                <template slot-scope="{row}">
                    <el-button-group>
                        ...
                        <el-button @click="$goto({path:'/admin/shitidetail',query:{id:row.id } })" icon="el-icon-info" type="info" size="mini"></el-button>
                        </el-tooltip>
                                            <el-tooltip content="编辑" placement="top">
                    <el-button icon="el-icon-edit" @click="$goto({path:'/admin/shitiupdt',query:{id:row.id } })"
    ...
    export default {
        ...
        methods: {
            ...
            loadList( page ){
                ...
                this.$post(api.shiti.listfaburen , filter).then(res=>{
                    this.loading = false;
            ...
            deleteItem( row ){
                this.$confirm('确定删除数据？' , '提示',{ // 弹出 确认框提示是否要删除
                    ...
                    this.$post(api.shiti.delete , {// 提交后台
                        id:row.id
                    }).then(res=>{
                        this.loading = false;
                        if(res.code != api.code.OK){
