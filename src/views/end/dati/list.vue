  ...
  <div class="v-list" v-loading="loading" element-loading-text="加载中">
    ...
    <el-table border :data="list" style="width: 100%" highlight-current-row>
      ...
      <el-table-column label="操作" align="center">
        <template slot-scope="{ row }">
          <el-button
            @click="$goto('/end/datipingyueadd?id=' + row.id)"
            type="text"
          >
            评阅
          </el-button>
          ...
          <el-button
            @click="$goto({ path: '/end/datidetail', query: { id: row.id } })"
            type="text"
            >详情</el-button
          >

          <el-button
            @click="$goto({ path: '/end/datiupdt', query: { id: row.id } })"
            type="text"
            >编辑</el-button
          >
...
export default {
  ...
  methods: {
    ...
    loadList(page) {
      ...
      this.$post(api.dati.list, filter)
        .then((res) => {
          this.loading = false;
