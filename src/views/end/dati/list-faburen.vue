<template>
        ...
        <el-table border :data="list" style="width: 100%" highlight-current-row>
            ...
            <el-table-column label="操作">
                <template slot-scope="{row}">
                    <el-button-group>
                    <el-button @click="$goto('/admin/datipingyueadd?id='+row.id)" type="success" icon="el-icon-plus" size="mini">
                                评阅
                            </el-button>

                                            <el-tooltip content="详情" placement="top">
                        <el-button @click="$goto({path:'/admin/datidetail',query:{id:row.id } })" icon="el-icon-info" type="info" size="mini"></el-button>
                        </el-tooltip>
                                            <el-tooltip content="编辑" placement="top">
                    <el-button icon="el-icon-edit" @click="$goto({path:'/admin/datiupdt',query:{id:row.id } })"
    ...
    export default {
        ...
        methods: {
            ...
            loadList( page ){
                ...
                this.$post(api.dati.listfaburen , filter).then(res=>{
                    this.loading = false;
            ...
            deleteItem( row ){
                this.$confirm('确定删除数据？' , '提示',{ // 弹出 确认框提示是否要删除
                    ...
                    this.$post(api.dati.delete , {// 提交后台
                        id:row.id
                    }).then(res=>{
                        this.loading = false;
                        if(res.code != api.code.OK){
