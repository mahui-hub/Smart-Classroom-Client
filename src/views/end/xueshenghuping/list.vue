<template>
        ...
        <div class="form-search">
            <el-form @submit.prevent.stop :inline="true" size="mini">
                ...
                </el-form-item>                <el-form-item label="互评分数">
...
-<el-input type="text" style="width:80px;" v-model="search.hupingfenshu_end"></el-input>
        ...
        <el-table border :data="list" style="width: 100%" highlight-current-row>
            <el-table-column type="index" label="#"></el-table-column> <!-- 序号 -->
                ...
                <template slot-scope="{row}">
                    {{  row.hupingneirong  }}                </template>
            </el-table-column>
                        <el-table-column label="互评分数"width="80">
                <template slot-scope="{row}">
                    {{  row.hupingfenshu  }}                </template>
            </el-table-column>
                        <el-table-column label="互评人"width="80">
                <template slot-scope="{row}">
                    {{  row.hupingren  }}                </template>
            </el-table-column>
            ...
            <el-table-column label="操作">
                <template slot-scope="{row}">
                    <el-button-group>
                        ...
                        <el-button @click="$goto({path:'/admin/xueshenghupingdetail',query:{id:row.id } })" icon="el-icon-info" type="info" size="mini"></el-button>
                        </el-tooltip>
                                            <el-tooltip content="编辑" placement="top">
                    <el-button icon="el-icon-edit" @click="$goto({path:'/admin/xueshenghupingupdt',query:{id:row.id } })"
    ...
    export default {
        ...
        methods: {
            ...
            loadList( page ){
                ...
                this.$post(api.xueshenghuping.list , filter).then(res=>{
                    this.loading = false;
            ...
            deleteItem( row ){
                this.$confirm('确定删除数据？' , '提示',{ // 弹出 确认框提示是否要删除
                    ...
                    this.$post(api.xueshenghuping.delete , {// 提交后台
                        id:row.id
                    }).then(res=>{
                        this.loading = false;
                        if(res.code != api.code.OK){
