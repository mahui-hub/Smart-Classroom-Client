    ...
    <div class="form-search">
      <el-form :model="search" :inline="true" size="mini">
        <el-form-item label="帖子分类">
          ...
          <!-- <el-select v-model="search.fenleimingcheng"
...
export default {
  ...
  methods: {
    submit() {
      this.$refs.formModel
        ...
        .then((res) => {
          ...
          this.$post(api.tiezifenlei.insert, form)
            .then((res) => {
              this.loading = false;
              if (res.code == api.code.OK) {
                this.$message.success("添加帖子类型成功");
    ...
    loadInfo() {
      ...
      // 获取模块得数据
      this.loading = true;
      this.$post(api.tiezifenlei.create, {
    ...
    operChange() {
      if (this.oper == "add") {
        this.headerTitle = "添加帖子类型";
      } else {
        this.headerTitle = "编辑帖子类型";
      }
    },
    submit1() {
      this.$refs.formModel
        ...
        .then((res) => {
          ...
          this.$post(api.tiezifenlei.update, form)
            .then((res) => {
              this.loading = false;
              if (res.code == api.code.OK) {
    ...
    loadInfo1() {
      ...
      this.loading = true;
      var form = this.form;
      this.$post(api.tiezifenlei.edit, {
        id: this.form.id,
      })
        .then((res) => {
          this.loading = false;
    ...
    loadList(page) {
      ...
      this.$post(api.tiezifenlei.list, filter)
        .then((res) => {
          this.loading = false;
          if (res.code == api.code.OK) {
            extend(this, res.data);
            this.tiezifenleilist = res.data.list;
          } else {
            this.$message.error(res.msg);
          }
        })
    ...
    deleteItem(row) {
      this.$confirm("确定删除数据？", "提示", {
        ...
        .then(() => {
          ...
          this.$post(api.tiezifenlei.delete, {
            // 提交后台
            id: row.id,
          })
            .then((res) => {
